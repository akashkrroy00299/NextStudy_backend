...
export const todayClasses = async (req, res) => {
  try {
    ...
    const todayClasses = classes.map((cls) => ({
      ...cls.toObject(),
      attended: attendanceByClassId.has(cls._id.toString()),
    }))
...
export const toggleAttended = async (req, res) => {
  try {
    ...
    if (attendanceLog) {
      ...
      const markedDate = startOfDay
      const now = new Date()
      await cacheDelMany([
        KEYS.gridData(userId, ttId, markedDate.getUTCMonth(), markedDate.getUTCFullYear()),
        KEYS.gridData(userId, ttId, now.getUTCMonth(), now.getUTCFullYear()),
    ...
    await cacheDelMany([
      KEYS.gridData(userId, ttId, startOfDay.getUTCMonth(), startOfDay.getUTCFullYear()),
      KEYS.gridData(userId, ttId, new Date().getUTCMonth(), new Date().getUTCFullYear()),
      KEYS.target(userId, ttId),
      KEYS.totalDashbord(userId, ttId),
    ])
